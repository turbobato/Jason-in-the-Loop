use crate::{components::*, player::{PlayerAnimations, PLAYER_SCALE, PLAYER_DIMENSIONS}};
use bevy::prelude::*;
...
fn player_loop_record_system(
    ...
    let (entity_id, transform, mut recording_on, recording_option) = query.get_single_mut().unwrap();
    ...
    if recording_on.0 {
        ...
        if let Some(recording) = recording_option {
            ...
            let clone_id = commands.spawn_bundle(SpriteSheetBundle {
                texture_atlas: animations.idle.clone(),
                transform: Transform {
                    translation: Vec3::new(recording.initial_pos.x, recording.initial_pos.y, recording.initial_pos.z),
